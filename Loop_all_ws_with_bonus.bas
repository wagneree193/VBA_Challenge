Attribute VB_Name = "Loop_all_ws_with_bonus"
Sub VBAStockTest()
...
For Each ws In Worksheets
 ...
 For i = 2 To lastrow
    ...
    max_inc = WorksheetFunction.Max(ws.Range("L:L"))
    max_dec = WorksheetFunction.Min(ws.Range("L:L"))
    max_vol = WorksheetFunction.Max(ws.Range("M:M"))
    ws.Cells(2, 17).Value = max_inc
    ws.Cells(3, 17).Value = max_dec
    ws.Cells(4, 17).Value = max_vol
    ws.Range("Q2:Q3").NumberFormat = "0.00%"
    ws.Range("Q4:Q10").NumberFormat = "0.00"
    
        If ws.Cells(i, 12).Value = max_inc Then
         ws.Cells(2, 16).Value = ws.Cells(i, 10).Value
         
        ElseIf ws.Cells(i, 12).Value = max_dec Then
